...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Plot some critical lines alive",
)
...
class AmassMotionCfg(AmassMotionCfgBase):
    ...
    path = "/home/xinyu/Projects/HKUST/Instinct/Datasets/lafan1_g1_instinct"
    ...
    filtered_motion_selection_filepath = "/home/xinyu/Projects/HKUST/Instinct/Datasets/lafan1_g1_instinct/type_walk_all.yaml"
...
def run_simulator(sim: SimulationContext, scene: InteractiveScene):
    ...
    if args_cli.video is not None:
        ...
        _render_product = rep.create.render_product(
            VIEWER_CFG.cam_prim_path, VIEWER_CFG.resolution
        )
    ...
    while simulation_app.is_running():
        ...
        robot.write_root_velocity_to_sim(
            torch.zeros(robot.num_instances, 6, device=torch.device("cuda"))
        )
        ...
        if reset_mask.any():
            ...
            print(
                "[INFO] current motion",
                motion_reference.get_current_motion_identifiers(reset_env_ids),
            )
        ...
        if args_cli.video is not None:
            if _video_interval_counter % DECIMATION == 0:
                ...
                if rgb_data.size == 0:
                    rgb_data = np.zeros(
                        (VIEWER_CFG.resolution[1], VIEWER_CFG.resolution[0], 3),
                        dtype=np.uint8,
                    )
                else:
                    rgb_data = rgb_data[:, :, :3]
        ...
        if args_cli.live_plot:
            if _plotter_counter % DECIMATION == 0:
                _, robot_pitch, robot_yaw = math_utils.euler_xyz_from_quat(
                    motion_reference_frame.base_quat_w[:, 0]
                )
